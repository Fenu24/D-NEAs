...
# taking name from the full name (eg. taking '1988 XB' from '  7753 (1988 XB)')
...
if path.exists("jpl_A2_sigma.txt"):
    ...
    for i in range(len(jpl)):
        print(i)
        spikd=int(int(jpl['spkid'][jpl.index[i]]))
        page='https://ssd-api.jpl.nasa.gov/sbdb.api?spk='+str(spikd)
        data = requests.get(page).json()
        
        for j in range(len(data['orbit']['model_pars'])):
            if data['orbit']['model_pars'][j]['name']=='A2':
                ind=j
...
#comapring JPL and cleared Marco's database
...
for i in range(len(jpl_number)):
    ...
    for j in range(len(marco_number)):
        if (jpl_number[i]==marco_number[j] and jpl_number[i]!='NaN') or jpl_name[i]==marco_name[j]:
            not_exist=i
            exist=j
...
dadt_jpl=np.empty(len(number))
dadt_papers=np.empty(len(number))
dadt_jpl[:] = np.NaN
dadt_papers[:] = np.NaN

dadt_sigma_jpl=np.empty(len(number))
dadt_sigma_papers=np.empty(len(number))
dadt_sigma_jpl[:] = np.NaN
dadt_sigma_papers[:] = np.NaN


for i in range(len(marco_number)):
    if i in exist_marco:
        ind=np.where(i==exist_marco)[0][0]
        dadt_jpl[i]=jpl_dadt[exist_jpl[ind]]
        dadt_sigma_jpl[i]=jpl_dadt_sigma[exist_jpl[ind]]
        
        
dadt_jpl[len(marco_number):]=jpl_dadt[new]
dadt_sigma_jpl[len(marco_number):]=jpl_dadt_sigma[new]

dadt_papers[:len(marco_number)]=marco_dadt
dadt_sigma_papers[:len(marco_number)]=marco_dadt_sigma
...
for i in range(len(data)):
    period1=data[i][145:158]
    period1=period1.replace(' ','')
    lcdb_min.append((data[i][177:181]).replace(' ',''))
    lcdb_max.append((data[i][182:186]).replace(' ',''))
    period1=period1.replace(' ','')
    number1=data[i][0:7]
...
P_lcdb=np.array([np.nan for _ in range(len(number))])

spin_axes_flag=np.array([' ' for _ in range(len(number))])
...
for i in range(len(name)):
    ...
    if name[i] in lcdb_name:
        ...
        if lcdb_P[lcdb_name.index(name[i])]!=P[i]:
            P_lcdb[i]=lcdb_P[lcdb_name.index(name[i])]
            
    
    elif number[i] in lcdb_number:
...
#            start.append(spin_start[spin_name.index(name[i])])
...
snr_max=np.zeros(len(snr_papers))
...
#    if selection_papers[i] or selection_jpl[i]:
